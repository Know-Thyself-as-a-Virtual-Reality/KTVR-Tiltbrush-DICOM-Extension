...
//        clipPlane.transform.localScale = new Vector3(0.15f * this.transform.localScale[0], 0.15f * this.transform.localScale[1], 0.15f * this.transform.localScale[2]);
    ...
    public int[] SetDimSize()
    {
        Regex rx = new Regex(@"DimSize = (\d{2,3}) (\d{2,3}) (\d{2,3})\n");

        string text = System.IO.File.ReadAllText(Path.Combine(imageDir, RawFileName.Replace(".raw", ".mhd")));

        MatchCollection matches = rx.Matches(text);

        int[] tempDim = new int[3] { 0, 0, 0 };
        ...
        if (matches.Count == 1)
        {
            ...
            try
            {
                tempDim[0] = Int32.Parse(groups[1].Value);
                tempDim[1] = Int32.Parse(groups[2].Value);
                tempDim[2] = Int32.Parse(groups[3].Value);
            }
            catch (Exception e)
            {
                Debug.Log(e);
                tempDim = new int[3] {256,256,256};
            }
        }else
        {
            tempDim = new int[3] { 256, 256, 256 };
